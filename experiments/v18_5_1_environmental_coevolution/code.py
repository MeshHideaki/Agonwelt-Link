# experiments/v18_5_1_environmental_coevolution/code.py
...
BASE_INHERITANCE_DECAY = 0.986

NOVELTY_BURST_PROB = 0.05
...
def generate_environment_tag(env):
    ...
    if (
        env["divergence_pressure"] > 0.66
        and
        env["novelty_pressure"] > 0.48
    ):
...
def rewire(
    ...
    for n in nodes:
        ...
        exploration_bias = 0.34
        persistence_bias = 0.56
        diversity_bias = 0.26

        if env_tag == "chaotic_environment":

            exploration_bias = 0.56
            persistence_bias = 0.34
            diversity_bias = 0.46

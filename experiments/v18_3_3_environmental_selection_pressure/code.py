# experiments/v18_3_environmental_selection_pressure/code.py
...
def rewire(nodes, env_tag):
    ...
    for n in nodes:
        ...
        if env_tag == "chaotic_environment":
            ...
            persistence_bias = 0.20

        elif env_tag == "stable_environment":

            exploration_bias = 0.12
            persistence_bias = 0.82

        elif env_tag == "isolated_region":

            exploration_bias = 0.55
            persistence_bias = 0.35
...
def compute_environmental_fitness(
    env_state,
    persistence_score,
    continuity_strength,
    anomaly_ratio,
    ...
    diversity_score = clamp(
        env_state["novelty_pressure"]
        * 0.5
        +
        env_state["local_entropy"]
        * 0.5
    )
...
def run_simulation(seed):
    ...
    reusable_memory = deque(
        maxlen=180
    )

    environmental_fitness_history = []

    adaptive_decay_history = []
    ...
    for step in range(STEP_COUNT):
        ...
        fitness = compute_environmental_fitness(
            env_state,
            persistence_score,
            continuity_strength,
            anomaly_ratio,
        ...
        environmental_fitness_history.append(
            fitness
        )
        ...
        reusable_memory.append(
            signature
        )

        dynamic_decay_threshold = (
        ...
        for n in nodes:
            ...
            n.environmental_fitness = fitness

            if (
                n.environmental_fitness
                <
                dynamic_decay_threshold
            ):
    ...
    average_environmental_fitness = float(
        np.mean(
            environmental_fitness_history
        )
    )

    topology_survival_rate = float(
        np.mean([
            n.environmental_fitness
            for n in nodes
        ])
    ...
    ) if adaptive_decay_history else 0.0

    recovery_success_rate = float(
        np.mean(recovery_history)
    )

    adaptive_selection_variance = float(
        np.var(
            environmental_fitness_history
        )
    )

    reusable_structure_ratio = (
        reusable_hits
        /
        (
            len(signatures)
    ...
    validation_result = all([
        ...
        0.0 <=
        average_environmental_fitness
        <= 1.0,

        0.0 <=
        topology_survival_rate
        <= 1.0,

        adaptive_decay_rate > 0.003,
        ...
        reusable_structure_ratio > 0.08,

        adaptive_selection_variance > 0.0005,

# Agonwelt-Link v19.1.7
# stabilized divergence floor recovery
...
def update_local_ecology(nodes):
    ...
    for node in nodes:
        ...
        # stronger divergence persistence
        node.region_signature += (
            divergence_force * 0.018
        )

        # ultra weak convergence
        node.region_signature += (
            local_alignment
            - node.region_signature
        ) * 0.0032
        ...
        # adaptive divergence floor
        if global_divergence < 0.24:

            if global_offset < 0.14:
        ...
        # anti-collapse divergence injection
        if global_divergence < 0.14:
